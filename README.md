...
host _public_ keys and the user's `authorized_keys` file can be written either
during the build or at runtime. At runtime, the env var `tunnel_authorized_keys`
will be written to `/home/tunnel/.ssh/authorized_keys`.
...
var `ssh_host_ed25519_key` will be written to `/etc/ssh/ssh_host_ed25519_key`
and its corresponding public key `ssh_host_ed25519_key_pub` will be written to
